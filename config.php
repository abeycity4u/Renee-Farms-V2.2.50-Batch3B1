...
// Central application timezone. Business dates across PHP, MySQL session date
// functions, and calendar defaults must agree at midnight. Override with the
// APP_TIMEZONE environment variable when a deployment operates elsewhere.
if (!defined('APP_TIMEZONE')) {
    $configuredTimezone = trim((string)(getenv('APP_TIMEZONE') ?: 'Africa/Lagos'));
    try {
        new DateTimeZone($configuredTimezone);
        define('APP_TIMEZONE', $configuredTimezone);
    } catch (Throwable $e) {
...
if (!function_exists('app_current_month')) {
    function app_current_month(): string { return date('Y-m'); }
}
if (!function_exists('app_timezone_name')) {
    function app_timezone_name(): string { return APP_TIMEZONE; }
}
...
if (!headers_sent()) {
    ...
    // V2.3 CSP enforcement rollout: retain the legacy emitter call temporarily.
    // The centralized alias emits enforcing CSP and protects mixed OPcache generations.
...
try {
    ...
    // Keep MySQL CURDATE()/NOW() aligned with the application's business date.
    // A numeric UTC offset works even when MySQL timezone-name tables are absent.
    $appOffset = (new DateTimeImmutable('now', new DateTimeZone(APP_TIMEZONE)))->format('P');
