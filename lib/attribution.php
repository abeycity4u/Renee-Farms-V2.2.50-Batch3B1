...
/**
 * Human-readable production/cycle scope for transaction tables.
 * Production type is the ownership level; a NULL cycle remains a deliberate
 * shared/no-specific-cycle attribution inside that production type.
 */
function attribution_production_label(string $farmType, ?string $productionType): string
{
    $farmType = strtolower(trim($farmType));
    $productionType = attribution_normalize_production_type($farmType, $productionType);
    $labels = attribution_production_types($farmType);
    if ($productionType === 'shared') {
        return $farmType === 'poultry' ? 'Shared Poultry' : ($farmType === 'ruminant' ? 'Shared Ruminant' : 'Shared');
    }
    return $labels[$productionType] ?? attribution_label($productionType);
}

function attribution_cycle_label(string $farmType, ?string $productionType, ?string $cycleCode): string
{
    $cycleCode = trim((string)$cycleCode);
    if ($cycleCode !== '') return $cycleCode;

    $farmType = strtolower(trim($farmType));
    $productionType = attribution_normalize_production_type($farmType, $productionType);
    if ($productionType === 'shared') {
        return $farmType === 'poultry' ? 'Shared between poultry cycles' : ($farmType === 'ruminant' ? 'Shared between ruminant cycles' : 'No specific cycle');
    }

    return 'Shared between ' . attribution_production_label($farmType, $productionType) . ' cycles';
}
