...
/**
 * Convert a Layer egg sale quantity to the egg ledger's canonical unit (crates).
 * Historical sales had no explicit UOM but the pre-UOM ledger already treated
 * their quantity as crates, so NULL/blank preserves that established meaning.
 * Returns null for a unit that has no safe egg-to-crate conversion.
 */
function layer_egg_sale_quantity_crates(float $quantity, ?string $unit): ?float
{
    $normalized = strtolower(trim((string)$unit));
    if ($normalized === '' || in_array($normalized, ['crate','crates','tray','trays'], true)) {
        return $quantity;
    }
    if (in_array($normalized, ['piece','pieces','egg','eggs'], true)) {
        return $quantity / LAYER_EGGS_PER_CRATE;
    }
    if (in_array($normalized, ['dozen','dozens'], true)) {
        return ($quantity * 12.0) / LAYER_EGGS_PER_CRATE;
    }
    return null;
}
...
function layer_egg_pool_before_sale(PDO $pdo, int $farmId, string $saleDate, int $saleId): array
{
    ...
    $sales = $pdo->prepare("SELECT id,sale_date,cycle_id,product_type,quantity,unit_of_measure
                            FROM sales_records
    ...
    foreach ($sales->fetchAll(PDO::FETCH_ASSOC) as $sale) {
        ...
        $convertedQty = layer_egg_sale_quantity_crates(max(0.0,(float)$sale['quantity']), $sale['unit_of_measure'] ?? null);
        // Unknown units are intentionally excluded from the derived crate pool rather than guessed.
        if ($convertedQty === null) continue;
        $saleQty = max(0.0,$convertedQty);
