...
$historicalFeedItemNames = [];
$historicalFeedStmt = $pdo->prepare("SELECT id, item_name FROM stock_items WHERE farm_id = ?");
$historicalFeedStmt->execute([$tenantFarmId]);
foreach ($historicalFeedStmt->fetchAll(PDO::FETCH_ASSOC) as $feedRow) {
    $historicalFeedItemNames[(int)$feedRow['id']] = (string)$feedRow['item_name'];
}
$dailyRecordFeedItemLabel = static function ($record) use ($historicalFeedItemNames): string {
    $feedItemId = (int)($record['feed_item_id'] ?? 0);
    return ($feedItemId > 0 && isset($historicalFeedItemNames[$feedItemId]))
        ? $historicalFeedItemNames[$feedItemId]
        : 'Not assigned';
};
...
if ($cycleEnabled) {
    ...
    if ($canonicalWorkspaceStock !== null) {
        $layerClosingStock = $canonicalWorkspaceStock;
    }
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['save_record'])) {
        ...
        // V3 cycles use canonical population. Legacy cycles retain the previous-record chain.
        if ($cycleIdForSave !== null) {
            $canonicalExpected =
                ...
                <div class="card poultry-panel">
                    ...
                    <div class="card-body bg-light">
                        ...
                        <div class="card mb-4">
                            ...
                            <div class="card-body app-calendar-scroll <?php echo $selectedCycleId === 0 ? 'calendar-legacy' : ''; ?>">
                                <div class="row">
                                    <?php
                                    // Generate calendar
...
<div class="col-md-4 mb-3">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <small class="text-muted">Quick daily note only. Use Poultry → Health &amp; Treatment for structured vaccination, treatment and symptom history.</small>
                            </div>
                        </div>
 ...
 <script src="<?php echo BASE_URL; ?>/assets/js/edit-modal.js"></script>
    ...
    id="layerDailyConfig"
    hidden
    data-selected-cycle-id="<?php echo (int)$selectedCycleId; ?>"
    data-year-month="<?php echo htmlspecialchars(
        (string)$yearMonth,
