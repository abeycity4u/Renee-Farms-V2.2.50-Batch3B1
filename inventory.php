...
// Active cycles are offered only as optional attribution targets for USED
// General / Non-feed inventory. Shared movements intentionally remain cycle-less.
$inventoryCycleStmt = $pdo->prepare("SELECT id, cycle_code, farm_type, production_type FROM production_cycles WHERE farm_id=? AND status='active' ORDER BY start_date DESC,id DESC");
$inventoryCycleStmt->execute([$currentFarmId]);
$inventoryActiveCycles = $inventoryCycleStmt->fetchAll(PDO::FETCH_ASSOC) ?: [];
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['update_category_financial_type'])) {
        ...
        $financialType = trim((string)($_POST['category_financial_type'] ?? ''));
        if ($categoryId <= 0 || !inventory_financial_classification_is_valid($financialType)) {
            $_SESSION['error'] = 'Choose a valid category and Financial Type.';
            header('Location: inventory.php');
            exit();
        }
        try {
            ...
            $pdo->prepare('UPDATE stock_items SET financial_classification=? WHERE category_id=? AND farm_id=?')
                ->execute([$financialType, $categoryId, $currentFarmId]);
            $pdo->commit();
            $_SESSION['success'] = 'Category Financial Type updated. Existing transaction history was not rewritten.';
        } catch (Throwable $e) {
            if ($pdo->inTransaction()) $pdo->rollBack();
            $_SESSION['error'] = safeUserExceptionMessage($e, 'The category Financial Type could not be updated.');
        }
        header('Location: inventory.php');
        exit();
    }
    ...
    if (isset($_POST['add_item'])) {
        ...
        try {
            ...
            $stmt = $pdo->prepare("INSERT INTO stock_items
                ...
                VALUES (?, ?, ?, 0, ?, ?, ?, ?, ?, ?, 0)");
            $stmt->execute([$currentFarmId, $itemName, $categoryId, $minStock, $unit, $farmType, $feedCategory, $defaultProductionType, $financialClassification]);
            ...
            if ($initialStock > 0) {
                stock_apply_movement($pdo, $currentFarmId, $itemId, 'received', $initialStock, $receivedDate,
                    ...
                    null, 'inventory_manual', null, $unitCost, $defaultProductionType);
            }
            $pdo->commit();
    ...
    if (isset($_POST['update_stock'])) {
        ...
        try {
            ...
            $pdo->beginTransaction();
            $movementItemStmt = $pdo->prepare("SELECT si.farm_type, si.feed_category, COALESCE(NULLIF(si.default_production_type,''),'shared') AS default_production_type, COALESCE(ic.financial_type, si.financial_classification, 'other_stock') AS financial_type FROM stock_items si JOIN inventory_categories ic ON ic.id=si.category_id AND ic.farm_id=si.farm_id WHERE si.id=? AND si.farm_id=? LIMIT 1");
            $movementItemStmt->execute([$itemId, $currentFarmId]);
            $movementItem = $movementItemStmt->fetch(PDO::FETCH_ASSOC);
            if (!$movementItem) {
                throw new RuntimeException('Inventory item not found.');
            }
            if ($type === 'received') {
                $financialClassification = (string)($movementItem['financial_type'] ?? 'other_stock');
                if (!inventory_financial_classification_is_valid($financialClassification)) {
                    throw new RuntimeException('This item category needs a valid Financial Type before stock can be received.');
                }
                if ((string)$movementItem['feed_category'] !== 'general' && $financialClassification !== 'feed') {
                    throw new RuntimeException('Feed usage must use an Inventory Category whose Financial Type is Feed.');
                }
...
<head>
    ...
    <link rel="stylesheet" href="<?php echo BASE_URL; ?><?php echo versioned_asset('/assets/css/inventory-page.css'); ?>">
</head>
<body>
...
<div class="card inventory-table-card">
                ...
                <div class="card-body p-0">
                    <div class="table-wrap table-responsive border-0 rounded-0">
                        <table id="inventoryTable" class="table table-hover mb-0 inventory-table align-middle">
                            ...
                            ?>
                            <tr>
                                <td><strong><?= htmlspecialchars($item['item_name'] ?? '-') ?></strong><div class="small text-muted"><?= htmlspecialchars($item['feed_category'] ?? 'general') ?><?php if (($item['feed_category'] ?? 'general') === 'general'): ?> · <?= htmlspecialchars(attribution_label($item['default_production_type'] ?? 'shared')) ?><?php endif; ?></div></td>
                        ...
                        <div class="modal-body">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-12 mb-3" id="defaultProductionTypeWrap">
                                    ...
                                    <select name="default_production_type" id="defaultProductionType" class="form-select"></select>
                                    <small class="text-muted">For General / Non-feed stock, choose where purchase spending should normally appear. Shared stock remains farm/module-level until usage is attributed.</small>
                                </div>
                            </div>
                    ...
                    <form method="POST" id="updateStockForm">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="date" name="transaction_date" class="form-control" value="<?php echo date('Y-m-d'); ?>" max="<?php echo date('Y-m-d'); ?>" required>
                            ...
                            <div class="mb-3 d-none" id="transactionCycleWrap">
                                ...
                                <small class="text-muted">Choose a cycle only when this usage belongs directly to one cycle. Leave blank for pooled/shared production-type usage.</small>
                            </div>

                            <div class="mb-3" id="unitCostWrapper">
                                <label>Unit Cost (₦) for Received Stock</label>
                                <input type="number" name="unit_cost" class="form-control" step="0.01" min="0" placeholder="0.00" required>
                                <small class="text-muted">Required for receipts. The actual purchase price is snapshotted on this receipt and used by inventory costing.</small>
                            </div>
 ...
 <script src="<?php echo BASE_URL; ?>/assets/js/edit-modal.js"></script>
    ...
    <div
        id="inventoryPageConfig"
        hidden
        data-active-cycles="<?php echo htmlspecialchars(
            app_json_script($inventoryActiveCycles),
            ENT_QUOTES | ENT_SUBSTITUTE,
            'UTF-8'
        ); ?>"
        data-base-url="<?php echo htmlspecialchars(
            BASE_URL,
            ENT_QUOTES | ENT_SUBSTITUTE,
            'UTF-8'
        ); ?>"
    ></div>
    <script src="<?php echo BASE_URL; ?><?php echo versioned_asset('/assets/js/inventory.js'); ?>"></script>
</body>
</html>
