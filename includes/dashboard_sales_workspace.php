...
ob_start(static function (string $html) use (
    ...
    if ($canViewSales) {
        $receivableAction = $canViewReceivables
            ? '<a class="btn btn-outline-light btn-sm" href="' . htmlspecialchars(rtrim(BASE_URL, '/') . '/management/sales_records.php', ENT_QUOTES, 'UTF-8') . '"><i class="bi bi-wallet2 me-1"></i> Receivables</a>'
            : '';
        $workspace = '\n        <div class="card dashboard-card mb-4" id="salesRepresentativeWorkspace">\n'
            ...
            . '                    <div class="text-muted small">Record and review farm produce sales' . ($canViewReceivables ? ', customer balances and debt payments.' : '.') . '</div>\n'
            . '                </div>\n'
            . '                <div class="d-flex gap-2 flex-wrap">\n'
            . '                    <a class="btn btn-success btn-sm" href="' . htmlspecialchars(rtrim(BASE_URL, '/') . '/management/sales_records.php', ENT_QUOTES, 'UTF-8') . '"><i class="bi bi-graph-up-arrow me-1"></i> Open Sales</a>\n'
            . '                    ' . $receivableAction . '\n'
            . '                </div>\n'
            . '            </div>\n'
            . '        </div>\n';
