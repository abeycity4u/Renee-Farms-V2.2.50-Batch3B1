 ...
 *
 * Read-only and database-free. It proves that checkout pricing/provider
 * architecture exists but remains deliberately unconfigured until commercial
 * price and provider decisions are approved.
 */
...
$check(($book['version'] ?? null) === ''
    ...
    'Stage 2A ships with no invented price, currency or commercial price-book version');
$check(billing_pricing_is_configured() === false,
    'pricing contract fails closed while the commercial price book is unconfigured');
...
$check($pricingRejected,
    'attempting to resolve a price before configuration is rejected');
...
$check(billing_pricing_bundle_key(['poultry']) === 'poultry'
    ...
    'pricing bundle identity supports Poultry, Ruminant, or both only');
...
$check($salesRejected,
    'shared Sales cannot become a separately priced module bundle');
...
$check(strpos($pricing, "['poultry', 'ruminant']") !== false
    && strpos($pricing, "return 'poultry+ruminant';") !== false,
    'pricing contract keeps livestock module scope separate from plan capacity');
$check(strpos($pricing, 'billing_pricing_price_book_hash') !== false
    && strpos($pricing, "hash('sha256'") !== false,
    'price book has deterministic SHA-256 identity for future audit linkage');
...
$check(strpos($pricing, '$packageRaw = $book[\'packages\'][$planCode][$bundleKey][$billingInterval]') !== false,
    'package price is keyed by plan, module bundle and billing interval');
$check(strpos($pricing, '$book[\'seat_unit_prices\'][$planCode][$role][$billingInterval]') !== false,
    'extra-seat unit price is keyed by plan, role and billing interval');
...
$check($pricedQuoteSignature,
    'authoritative priced-quote entry point accepts no caller amount or currency parameter');
...
$check(method_exists('BillingProviderAdapterInterface', 'initializeCheckout')
    ...
    'provider adapter surface is limited to checkout, payment verification and webhook verification');
$check(billing_provider_registered_codes() === [],
    'no payment provider adapter is registered before provider selection');
...
$check($providerRejected,
    'provider operations fail closed when no adapter is registered');
...
$check($paidWithoutVerificationRejected,
    'an unverified provider result can never normalize to paid');
...
$check($unsignedWebhookRejected,
    'webhook event is rejected unless provider signature verification succeeds');
...
$check($httpCheckoutRejected,
    'provider checkout redirect must be HTTPS');
...
$check(strpos($provider, "hash_equals($amount, billing_provider_normalize_amount($quote['amount'] ?? null))") !== false
    ...
    'provider checkout cross-checks pricing amount/currency against the frozen payment quote');
$check(strpos($provider, "'payload_hash' => hash('sha256', $rawPayload)") !== false,
    'webhook contract exposes only SHA-256 payload identity to persistence');
$check(strpos($provider, "['pending', 'paid', 'failed', 'cancelled', 'refunded']") !== false,
    'provider payment statuses normalize to the canonical Stage 1 state vocabulary');
...
$check(strpos($init, 'includes/billing_pricing_contract.php') === false
    ...
    'Stage 2A contracts are not globally loaded into application runtime yet');

$check(strpos($provider, 'grant') === false || stripos($provider, 'never grant') !== false,
    'provider contract does not expose an entitlement-grant operation');
...
echo "NOTE: commercial prices and the concrete payment provider are intentionally still unconfigured.\n";
