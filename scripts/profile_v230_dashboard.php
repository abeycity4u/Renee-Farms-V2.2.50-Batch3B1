...
if ($farmId <= 0) {
    $stmt = $pdo->prepare('SELECT id, farm_name FROM farms WHERE farm_name = ? LIMIT 2');
    ...
    $farmName = (string)$matches[0]['farm_name'];
} else {
    $stmt = $pdo->prepare('SELECT farm_name FROM farms WHERE id = ? LIMIT 1');
    $stmt->execute([$farmId]);
