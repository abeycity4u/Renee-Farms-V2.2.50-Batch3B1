...
$add(
    'Guest limiter identity is keyed by limiter key and remote IP',
    preg_match(
        '/\$guestIdentity\s*=\s*\$key\s*\.\s*[\'"]\|guest\|[\'"]\s*\.\s*\$ip\s*;/',
        $helpers
    ) === 1
);
...
$add(
    'Rate limit still returns HTTP 429 through shared JSON response',
    preg_match(
        '/send_json\s*\(\s*\[\s*[\'"]success[\'"]\s*=>\s*false\s*,\s*[\'"]error[\'"]\s*=>\s*[\'"]Too many requests\. Please try again shortly\.[\'"]\s*,?\s*\]\s*,\s*429\s*\)/s',
        $helpers
    ) === 1
);
